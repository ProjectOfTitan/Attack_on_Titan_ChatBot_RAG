...
CASES_ROOT = APP_ROOT / "cases"


st.set_page_config(page_title="AoT Quality Lab Dashboard", page_icon="📊", layout="wide")
st.title("AoT Quality Lab Dashboard")
st.caption("Quality Lab 하네스 실행 결과를 누적 비교하고, false positive와 실패 원인을 빠르게 추적하기 위한 평가 전용 대시보드입니다.")
...
def load_runs() -> list[dict[str, Any]]:
    ...
    for path in paths:
        ...
        if isinstance(cases_path, str) and not cases_path.startswith(str(CASES_ROOT)):
            continue
        ...
        rows.append(
            {
                ...
                "forbidden_title_ok": bool(checks.get("forbidden_title_ok", True)),
                "required_pairs_ok": bool(checks.get("required_pairs_ok", True)),
                "missing_parts": ", ".join(result.get("missing_parts", [])),
                "forbidden_parts_found": ", ".join(result.get("forbidden_parts_found", [])),
                "forbidden_title_matches": ", ".join(result.get("forbidden_title_matches", [])),
...
def render_case_result(result: dict[str, Any], heading: str) -> None:
    ...
    if result.get("forbidden_title_matches"):
        st.write("Forbidden Title Matches")
        st.write(result["forbidden_title_matches"])
    if result.get("failed_required_pairs"):
        st.write("Failed Required Pairs")
        st.json(result["failed_required_pairs"])
    st.write("Checks")
...
if not runs:
    ...
    st.code("/mnt/e/one_piece/venv/bin/python /mnt/e/one_piece/aot_quality_lab/harness.py --profile openrouter_gemma_free_existing_aot")
    st.stop()
...
if selected_run["ragas_summary"]:
    ...
    st.dataframe(ragas_df, width="stretch", hide_index=True)
...
with st.expander("Run History Table", expanded=False):
    ...
    st.dataframe(display_history_df, width="stretch", hide_index=True)
...
st.dataframe(display_case_df, width="stretch", hide_index=True)
